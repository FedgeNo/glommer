...
### 1. Reply and mention fan-out
Delivery reaches the author's followers and nobody else, so a reply to a
Mastodon post never reaches the person replied to unless they already follow
you, and a mention reaches nobody. The parent's author and any mentioned actors
have to join the delivery set, and appear in the Note's `to`/`cc` and `tag`.
This is a correctness bug, not a feature gap.

### 2. Likes and Announce
Inbound `Like` and `Announce` (and their `Undo`s) against local posts; outbound
when a member likes or shares. Needs a table to remember remote likes and boosts
so an `Undo` can find what it is undoing. **Schema change.**
...
### 4. Flag (federated reports), inbound and outbound
...
### 5. Instance-level blocking / defederation
...
### 11. Video and Audio as first-class objects
