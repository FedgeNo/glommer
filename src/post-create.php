...
if (defined('IS_API_POST_REQUEST')) {
    ...
    echo json_encode([
        ...
        'url' => $url,
        'content' => $content,
        'in_reply_to_id' => $parent_id === null ? null : (string) $parent_id,
        'sensitive' => $sensitive === 1,
        'spoiler_text' => $content_warning ?? '',
        'visibility' => 'public',
        'language' => $detected_language,
        'account' => [
            'id' => (string) $current_user -> userId,
            'username' => $current_user -> slug,
            'acct' => $current_user -> slug,
            'display_name' => $current_user -> title ?? $current_user -> slug,
            'url' => ActivityPubActor::uriFor($current_user),
        ],
    ], JSON_THROW_ON_ERROR);
    exit;
}
