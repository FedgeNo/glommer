...
export default {
    ...
    tests: {
        'the list is built over the notice standing in its place'() {
            ...
            container.innerHTML = '<p class="Notice">No blocked servers.</p>';

            const list = list_in(container, 'BlockedServerList');

            TestCase.assertNotNull(list);
            TestCase.assertEquals('UL', list.tagName);
            TestCase.assertEquals('BlockedServerList', list.className);
        ...
        'an existing list is handed back untouched'() {
            ...
            existing.className = 'BlockedServerList';
