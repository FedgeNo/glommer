...
if ($environment_errors === [] && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['siteURL'])) {
    ...
    if (!CSRF::verify(is_string($csrf_token) ? $csrf_token : null)) {
        ...
        try {
            SetupClaim::configure(static function () use (&$errors, &$success): void {
                ...
                if ($errors === []) {
                    ...
                    $mask = umask(0077);
                    try {
                        $env_file = @fopen(__DIR__ . '/../.env', 'x');
                    } finally {
                        umask($mask);
                    }
                    $written = $env_file !== false && fwrite($env_file, $env_contents) === strlen($env_contents);
                    if ($env_file !== false) {
                        fclose($env_file);
                    }
                    if (!$written) {
