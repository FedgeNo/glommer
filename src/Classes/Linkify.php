 ...
 * one combined URL-or-hashtag scan run left-to-right with the URL alternative
 * first, and byte offsets sliced with byte-based substr (the JS side uses UTF-16
 * offsets sliced with slice - same result because the classes never match a
 * multibyte byte or a surrogate).
 *
 ...
 *     URLs become self-links, #hashtags become tag links, the rest stays text.
 */
class Linkify
{
    ...
    // (not a word char or another #, so a#b and ##b don't tag). URL first so a
    // '#' inside a URL's fragment never starts a hashtag. Shared verbatim with
    // delta.js via the same string; only the delimiter differs (PHP {} vs JS
    // new RegExp). No {} in the body so the {} delimiter is safe.
    private const SCAN = "https?://[A-Za-z0-9._~:/?#\\[\\]@!$&'()*+,;=%-]+|(?<![A-Za-z0-9_#])#[A-Za-z0-9_]+";
     ...
     * Each is ['type' => 'text'|'url'|'hashtag', 'text' => shown text, and for a
     * hashtag 'tag' => the lowercased tag]. Adjacent text segments are merged so
     * a run's formatting wraps one node per contiguous stretch (matching today's
     * output for URL/hashtag-free text exactly).
     *
