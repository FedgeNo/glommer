...
/**
 * Shortcode to emoji - the whole vocabulary, held here and nowhere else.
 *
 * Taken once from emojibase's GitHub set and hard-coded. There is no generator
 * and nothing fetches this at runtime: source that gets executed, in either
 * language, must never be written from content off the network, however
 * carefully the writing is reviewed. Changing the vocabulary is a deliberate
 * edit to this file.
 *
 * There is no authoritative list to defer to anyway. Unicode and CLDR define
 * emoji names and keywords, not colon codes, and the sets that do exist
 * disagree about the same character. GitHub's is what people have actually
 * typed for a decade, and the smallest, which matters because it is served to
 * the browser too - see javaScriptModule(), the one copy the client gets.
 *
 * The characters are byte escapes rather than literals, so the file is diffable
 * and survives any editor or terminal that would otherwise mangle an emoji it
 * cannot display.
 */
class EmojiShortcodeMap
{
    /**
     * The same table as a JavaScript module, for the client half of the
     * expansion.
     *
     * Served rather than kept as a second file: two copies of a vocabulary are
     * two things that can drift, and this way there is only ever one. The map
     * is data, so it is encoded as JSON rather than assembled as source - the
     * browser is handed a value, not something written for it to execute.
     */
    public static function javaScriptModule(): string
    {
        return "export const EMOJI_SHORTCODES = "
            . json_encode(self::MAP, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR)
            . ";\n";
    }

    /** Changes only when the table above does, so it is a usable ETag. */
    public static function version(): string
    {
        return substr(hash('sha256', (string) json_encode(self::MAP)), 0, 16);
    }
